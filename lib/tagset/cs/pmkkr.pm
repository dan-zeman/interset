...
sub encode
{
    ...
    # jiné = other (foreign, abbreviation, proper noun)
    elsif($f{foreign} eq 'foreign' || $f{abbr} eq 'abbr' || $f{subpos} eq 'prop')
    {
        $values[1] = 'J';
        # 2. skutečný druh: CZP
    ...
    # untagged tokens in multi-word expressions have empty tags like this:
    # <i1></i1><i2></i2><i3></i3><i4></i4>
    # This is also our default for unmatching feature structures from other tagsets.
    else
    {
...
# after adding tags resulting from stripping 'other': 244
#------------------------------------------------------------------------------
sub list
{
    my $list = <<end_of_list
