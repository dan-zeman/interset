#!/usr/bin/perl
# Driver for the long version of the tagset of the Pražský mluvený korpus (Prague Spoken Corpus) of Czech.
# Copyright © 2009, 2010 Dan Zeman <zeman@ufal.mff.cuni.cz>
# License: GNU GPL

package tagset::cs::pmkdl;
use utf8;
use tagset::common;
use tagset::cs::pmk;
...
sub decode
{
    ...
    if($pos==1)
    {
        ...
        # 3. třída
        tagset::cs::pmk::decode_noun_class($values[3], \%f);
        # 4. valence
        tagset::cs::pmk::decode_valency($pos, $values[4], '', \%f);
        # 5! rod
        tagset::cs::pmk::decode_gender($pos, $values[5], \%f);
        # 6. číslo
        tagset::cs::pmk::decode_number($pos, $values[6], \%f);
        # 7. pád
    ...
    elsif($pos==2)
    {
        ...
        # 4. třída
        tagset::cs::pmk::decode_adjective_class($values[4], \%f);
        # 5. valence
        tagset::cs::pmk::decode_valency($pos, $values[5], '', \%f);
        # 6. rod
        tagset::cs::pmk::decode_gender($pos, $values[6], \%f);
        # 7. číslo
        tagset::cs::pmk::decode_number($pos, $values[7], \%f);
        # 8. pád
        tagset::cs::pmk::decode_case($values[8], \%f);
        # 9. stupeň
    ...
    elsif($pos==3)
    {
        ...
        # 3. valence
        tagset::cs::pmk::decode_valency($pos, $values[3], '', \%f);
        # 4. rod
        tagset::cs::pmk::decode_gender($pos, $values[4], \%f);
        # 5. číslo
        tagset::cs::pmk::decode_number($pos, $values[5], \%f);
        # 6. pád
    ...
    elsif($pos==5)
    {
        $f{pos} = 'verb';
        # 2. druh
        tagset::cs::pmk::decode_aspect($values[2], \%f);
        # 3. valence subjektová
        # 4. valence
        tagset::cs::pmk::decode_valency($pos, $values[3], $values[4], \%f);
        # 5. osoba/číslo
        tagset::cs::pmk::decode_person_number($values[5], \%f);
        # 6. způsob/čas/slovesný rod
        tagset::cs::pmk::decode_mood_tense_voice($values[6], \%f);
        # 7. imper./neurč. tvary
        tagset::cs::pmk::decode_nonfinite_verb_form($values[7], \%f);
        # 8! víceslovnost a rezultativnost
        tagset::cs::pmk::decode_multiwordness_and_resultativeness($values[8], \%f);
        # 9. jmenný rod
        tagset::cs::pmk::decode_participle_gender_number($values[9], \%f);
        # 10! zápor
        tagset::cs::pmk::decode_negativeness($values[10], \%f);
        # 11! styl
        tagset::cs::pmk::decode_style($values[11], \%f);
    }
    # adverbium = adverb
    elsif($pos==6)
    {
        ...
        # 3. třída
        tagset::cs::pmk::decode_adverb_class($values[3], \%f);
        # 4. valence/funkce
        tagset::cs::pmk::decode_valency($pos, $values[4], '', \%f);
        # 5. stupeň
        tagset::cs::pmk::decode_degree($values[5], \%f);
        # 6! styl
    ...
    elsif($pos==7)
    {
        ...
        # 3. třída
        tagset::cs::pmk::decode_preposition_class($values[3], \%f);
        # 4. valenční pád
        tagset::cs::pmk::decode_case($values[4], \%f);
        # 5. funkční závislost levá
        tagset::cs::pmk::decode_function($pos, $values[5], \%f);
        # 6! styl
        tagset::cs::pmk::decode_style($values[6], \%f);
    }
    # spojka = conjunction
    elsif($pos==8)
    {
        ...
        # 3. třída
        tagset::cs::pmk::decode_conjunction_class($values[3], \%f);
        # 4. valence
        tagset::cs::pmk::decode_valency($pos, $values[4], '', \%f);
        # 5! styl
        tagset::cs::pmk::decode_style($values[5], \%f);
    }
    ...
    elsif($pos==9)
    {
        ...
        # 3. třída
        tagset::cs::pmk::decode_interjection_class($values[3], \%f);
        # 4! styl
    ...
    elsif($pos eq '0')
    {
        ...
        # 3. třída
        tagset::cs::pmk::decode_particle_class($values[3], \%f);
        # 4. valence
        tagset::cs::pmk::decode_valency($pos, $values[4], '', \%f);
        # 5. modus věty
    ...
    else # $pos eq 'J'
    {
        ...
        # P4. rod
        tagset::cs::pmk::decode_gender('P', $values[4], \%f);
        # P5. číslo
        tagset::cs::pmk::decode_number('P', $values[5], \%f);
        # P6. pád
        tagset::cs::pmk::decode_case($values[6], \%f);
        # P7! styl
        tagset::cs::pmk::decode_style($values[7], \%f);
    }
    return \%f;
}
...
sub encode
{
    ...
    else
    {
        ...
        # P4. rod
        $values[4] = tagset::cs::pmk::encode_gender('P', $f);
        # P5. číslo
        $values[5] = tagset::cs::pmk::encode_number('P', $f);
        # P6. pád
        $values[6] = tagset::cs::pmk::encode_case($f);
        # P7! styl
        $values[7] = tagset::cs::pmk::encode_style($f);
    }
    ...
    (
        ...
        5 => [1, 8, 10, 11], # verbs
        6 => [1, 2, 6], # adverbs
        7 => [1, 2, 6], # prepositions
        8 => [1, 2, 5], # conjunctions
        9 => [1, 2, 4], # interjections
        0 => [1, 2, 6], # particles
        'F' => [1, 2], # idioms
        'J' => [1, 2, 7], # other; the last number, 7, only relevant for the P subtype (proper nouns)
    );
...
<i1>0</i1><i2>5</i2><i3>1</i3><i4>2</i4>
...
<i1>2</i1><i2>0</i2><i3>1</i3><i4>1</i4>
...
<i1>2</i1><i2>1</i2><i3>_</i3><i4>1</i4>
<i1>2</i1><i2>1</i2><i3>_</i3><i4>2</i4>
...
<i1>5</i1><i2>2</i2><i3>3</i3><i4>1</i4>
...
<i1>7</i1><i2>3</i2><i3>3</i3><i4>3</i4>
...
<i1>F</i1><i2>1</i2><i3>2</i3><i4>-</i4>
<i1>F</i1><i2>1</i2><i3>5</i3><i4>-</i4>
<i1>F</i1><i2>1</i2><i3>6</i3><i4>-</i4>
<i1>F</i1><i2>1</i2><i3>7</i3><i4>2</i4>
<i1>F</i1><i2>1</i2><i3>7</i3><i4>4</i4>
<i1>F</i1><i2>1</i2><i3>8</i3><i4>3</i4>
...
<i1>F</i1><i2>4</i2><i3>-</i3><i4>_</i4>
<i1>F</i1><i2>4</i2><i3>1</i3><i4>_</i4>
<i1>F</i1><i2>4</i2><i3>4</i3><i4>_</i4>
<i1>F</i1><i2>4</i2><i3>5</i3><i4>_</i4>
<i1>F</i1><i2>4</i2><i3>9</i3><i4>_</i4>
<i1>F</i1><i2>4</i2><i3>_</i3><i4>1</i4>
<i1>F</i1><i2>4</i2><i3>_</i3><i4>2</i4>
<i1>F</i1><i2>5</i2><i3>1</i3><i4>_</i4>
<i1>F</i1><i2>5</i2><i3>2</i3><i4>-</i4>
<i1>F</i1><i2>5</i2><i3>2</i3><i4>_</i4>
...
<i1>J</i1><i2>P</i2><i3>_</i3><i4>1</i4>
