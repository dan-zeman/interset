#!/usr/bin/perl
# This is a modification of the original pdt.pm Module which works with the 16 positon tag format and is compatible with the ajka.pm module
# (c) 2009 Markus Schäfer
# Driver for the positional tagset of the Prague Dependency Treebank.
# (c) 2006 Dan Zeman <zeman@ufal.mff.cuni.cz>
# License: GNU GPL

package tagset::cs::pdt16;
use utf8;
use tagset::common;
...
sub encode
{
    ...
    # person
    if($f{person} eq "1,2,3")
    {
        $tag[7] = "X"; # ???
    }
    elsif($f{person}>=1 && $f{person}<=3)
    {
        $tag[7] = $f{person};
    }
    ...
    elsif($f{aspect} eq "imp")
    {
        $tag[15] = "I";
    }
    elsif($f{aspect} eq "perf,imp")
    {
        $tag[15] = "B";
    }
