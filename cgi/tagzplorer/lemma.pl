...
{
    print("<h1>Example Lemma: <span style='color:red'>$config{lemma}</span></h1>\n");
    my @positions;
    # Which index file do we need?
    $config{lemma} =~ m/^(.)/;
    my $fl = $1;
    my $indexpath = sprintf("$config{corpus}/lindex%04x.txt", ord($fl));
    ...
    while(<INDEX>)
    {
        ...
        if(m/^(.*?)\t(.*)$/)
        {
            my $lemma = $1;
            my $records = $2;
            if($lemma eq $config{lemma})
            {
                # Print all tags the lemma occurred with.
                if($config{tag})
                {
                    print("<p>Interested only in tag: $config{tag}. <a href=\"form.pl?corpus=$config{corpus}&amp;form=$config{form}\">Remove tag filter.</a></p>\n");
                }
                print("<table border=1>\n");
                print("<tr><th>Lemma</th><th>Tag</th><th>Form</th><th>Positions</th></tr>\n");
                # Parse records.
                ...
                foreach my $record (@records)
                {
                    ...
                    my $form = "<a href='form.pl?corpus=$config{corpus}&amp;form=".uri_escape_utf8($fields[0])."'>$fields[0]</a>";
                    ...
                    print("<tr><td>$lemma</td><td>$tag</td><td>$form</td><td>$positions</td></tr>\n");
                }
                print("</table>\n");
                last;
            }
    ...
    close(INDEX);
    # Print the first 10 sentences where the lemma occurs.
    print("<ul>\n");
